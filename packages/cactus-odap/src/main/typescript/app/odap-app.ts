...
import {
  OdapDummyInfrastructure,
  org1Env,
} from "./infrastructure/odap-backend";
import { DefaultApi as OdapApi } from "../../typescript/public-api";
...
export class SupplyChainApp {
  ...
  private readonly ledgers: OdapDummyInfrastructure;
  ...
  public constructor(public readonly options: ISupplyChainAppOptions) {
    ...
    this.ledgers = new OdapDummyInfrastructure({
      logLevel,
  ...
  public async start(): Promise<IStartInfo> {
    ...
    const addressInfoB = httpApiC.address() as AddressInfo;
    const nodeApiHostB = `http://localhost:${addressInfoB.port}`;

    const besuConfig = new Configuration({ basePath: nodeApiHostA });
    const fabricConfig = new Configuration({ basePath: nodeApiHostB });
